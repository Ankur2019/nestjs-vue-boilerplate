...
export type CreateUserData = PartialBy<UserWithoutId, 'isVerified' | 'role' | 'uid'>;
export default class UserModel {
  static readonly BaseModel = model<UserDocument>('users', UserSchema);

  static readonly SALT_ROUNDS = 10;

  static readonly UID_START = 1000;

  static async _getLatestUid(): Promise<number> {
    const userWithLatestUid = await UserModel.BaseModel
      ...
      .lean()
      .exec();
    return userWithLatestUid.length ? userWithLatestUid[0].uid : UserModel.UID_START;
  }
  ...
  static async createUser(createUserData: CreateUserData): Promise<ServerUser> {
    ...
    const latestUid = await UserModel._getLatestUid();
